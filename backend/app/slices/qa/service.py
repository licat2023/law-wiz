...
async def create_session(db: AsyncSession, *, user_id: int, payload: CreateSessionRequest) -> SessionData:
...
async def list_sessions(
    ...
    *,
    ...
    total = await db.scalar(select(func.count()).select_from(QaSession).where(*conditions)) or 0
    sessions = (
        (
            await db.execute(
                select(QaSession)
...
async def get_session(
    ...
    session = await _load_owned_session(db, user_id=user_id, session_id=session_id)
    if page < 1 or page_size < 1 or page_size > _MAX_PAGE_SIZE:
        raise BusinessError(
            ErrorCode.PAGE_OUT_OF_RANGE, f"分页参数超出范围（page_size 上限 {_MAX_PAGE_SIZE}）"
        )
    ...
    total = (
        await db.scalar(
            select(func.count()).select_from(QaMessage).where(QaMessage.qa_session_id == session.id)
        )
        or 0
    )
    messages = (
    ...
    citations = await _load_citations(db, [message.id for message in messages])
...
async def _load_citations(db: AsyncSession, message_ids: list[int]) -> dict[int, list[CitationItem]]:
    ...
    citations = (
        (await db.execute(select(QaCitation).where(QaCitation.qa_message_id.in_(message_ids))))
        .scalars()
        .all()
    )
    if not citations:
...
async def _load_owned_session(db: AsyncSession, *, user_id: int, session_id: int) -> QaSession:
...
async def ask(db: AsyncSession, *, user_id: int, session_id: int, content: str) -> AskData:
...
async def enqueue(*, message_id: int, session_id: int) -> None:
    """登记并执行回答生成。

    单独成函数是为了**让测试可以替换它** —— 测试里不应真的调用模型。
    """
    await run_answer_pipeline(message_id=message_id, session_id=session_id)
...
async def archive(db: AsyncSession, *, user_id: int, session_id: int) -> None:
