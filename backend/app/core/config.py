...
class Settings(BaseSettings):
    ...
    # PDF 解析页数上限。pypdf 逐页 `extract_text` 会把各页文本读入内存后一次性拼接，
    # 页数越多峰值内存越高 —— 20 MB 的 PDF 若页数极多，可在约 950 MiB 可用内存下 OOM。
    # 取值依据：审查场景是**合同**（不是整本法规），中文合同极少超过 100 页；
    # 知识库语料走 JSON（D-01）不经此处解析，不受此限。300 页对正常合同绰绰有余。
    max_pdf_pages: int = 300

    # ---------- 上传配额 ----------
    # 单次 20 MB 挡不住"反复上传"：一个用户可无限上传不同内容 → 磁盘耗尽。
    # 按**用户累计**设两道上限（每次新增内容前检查；去重命中不落盘、不占配额）：
    # - 字节数：直接对应磁盘占用；
    # - 文件数：防大量小文件耗尽 inode 与每行开销。
    # ⚠️ 报告 PDF 也是 file_object（数据模型无类型字段可区分），会计入用户用量；
    # 其体积很小，相对 200 MB 可忽略。
    # ⚠️ 超限复用 `41301`（文档中该码语义为"单个文件超标"）—— 属知情权衡，见风险清单 #6。
    upload_quota_bytes_per_user: int = 200 * 1024 * 1024  # 200 MB
    upload_quota_files_per_user: int = 50
